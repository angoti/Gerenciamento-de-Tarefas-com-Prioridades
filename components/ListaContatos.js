...
export default function ListaContatos({ contatos, editar, editando }) {
	...
	function ordenarContatos(contatos) {
		return contatos.sort((a, b) => {
			...
			return ordemAscendente ? prioridades[a.prioridade] - prioridades[b.prioridade] : prioridades[b.prioridade] - prioridades[a.prioridade];
		});
	}
	...
	function renderizarContato({ item, index }) {
		return (
			<Pressable style={styles.container} onPress={() => editar(index)}>
				...
				<View style={{ flex: 1 }}>{editando === index && <MaterialIcons name="edit" size={16} color="red" />}</View>
			</Pressable>
		);
	}
	...
	return (
		<View style={{ flex: 1 }}>
			...
			<FlatList style={styles.lista} data={ordenarContatos([...contatos])} renderItem={(item, index) => renderizarContato(item, index)} />
		</View>
	);
}
